...
#gcc -c `perl -MConfig -e 'print join(" ", @Config{qw(ccflags optimize cccdlflags)}, "-I$Config{archlib}/CORE")'`
gcc -c `perl -MConfig -e 'print join(" ", @Config{qw(ccflags optimize cccdlflags)}, "-I$Config{archlib}/CORE")'` -I/usr/local/Cellar/libressl/2.1.6/include/ -L/usr/local/Cellar/libressl/2.1.6/lib -lcrypto key_utils.c key_utils_wrap.c
perl Makefile.PL
make
make install
